...
import { ensureStartsWith } from "@/lib/shopify";
...
export const createCart = createServerFn().handler(async () => {
	...
	data.update({ cartId: res.body.data.cartCreate.cart.id });

	return reshapeCart(res.body.data.cartCreate.cart);
});

export type AddToCartFormData = {
	lines: { merchandiseId: string; quantity: number }[];
};

export const addToCart = createServerFn()
	.inputValidator((data: AddToCartFormData) => data)
...
export const removeFromCart = createServerFn()
	.inputValidator((data: { lineIds: string[] }) => data)
	.handler(async ({ data: { lineIds } }) => {
		const {
			data: { cartId },
		} = await useAppSession();
...
export const getMenu = createServerFn()
	.inputValidator((data: { handle: string }) => data)
	.handler(async ({ data: { handle } }) => {
...
export const getPages = createServerFn().handler(async () => {
...
export const getProductRecommendations = createServerFn()
